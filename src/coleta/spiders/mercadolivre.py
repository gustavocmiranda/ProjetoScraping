    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            yield {
                ...
                'ratings_amount': product.css('span.ui-search-reviews__amount::text').get(),
                'reais_antigo' : prices[0] if len(prices) > 0 else None,
                'centavos_antigo' : cents[0] if len(cents) > 0 else None,
                'reais_atual' : prices[1] if len(prices) > 1 else None,
